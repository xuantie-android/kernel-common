...
static inline void th1520_snd_txctrl(struct th1520_i2s_priv *chip, bool on)
{
	...
	if (on) {
		...
		dma_en &= ~DMACR_TDMAE_EN;
		ret = readl_poll_timeout_atomic(chip->regs + I2S_SR, i2s_status,
					       !(i2s_status & SR_TXBUSY_STATUS) &&
					       (i2s_status & SR_TFNF_TX_FIFO_NOT_FULL),
					       1, 100000);
		if (ret)
			dev_warn(chip->dev, "timed out waiting for TX idle\n");
		writel(dma_en, chip->regs + I2S_DMACR);
		/*
				    ...
				    struct snd_soc_dai *dai)
{
	...
	i2s_en = readl(i2s_private->regs + I2S_IISEN);
...
static const struct of_device_id th1520_i2s_of_match[] = {
	{ .compatible = "xuantie,th1520-ap-i2s", .data = AP_I2S },
...
static int th1520_audio_i2s_probe(struct platform_device *pdev)
{
	...
	match_name = of_device_get_match_data(dev);
	if (match_name) {
		strscpy(i2s_priv->name, match_name, sizeof(i2s_priv->name));
	} else if (strstr(pdev->name, AP_I2S)) {
		strscpy(i2s_priv->name, AP_I2S, sizeof(i2s_priv->name));
	}
	else if (strstr(pdev->name, AUDIO_I2S0)) {
		strscpy(i2s_priv->name, AUDIO_I2S0, sizeof(i2s_priv->name));
	}
	else if (strstr(pdev->name, AUDIO_I2S1)) {
		strscpy(i2s_priv->name, AUDIO_I2S1, sizeof(i2s_priv->name));
	}
	else if (strstr(pdev->name, AUDIO_I2S2)) {
		strscpy(i2s_priv->name, AUDIO_I2S2, sizeof(i2s_priv->name));
	}
	else {
	...
	ret = th1520_pcm_probe(pdev, i2s_priv, TH1520_I2S_DMABUF_SIZE);
	if (ret)
		goto err_pm_disable;
